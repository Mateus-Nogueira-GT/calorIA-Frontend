...
| DO4 | Bibliotecas nativas | `react-native-screens` **3.35 → 4.12.0 (pin exato)**. Obrigatório: a tabela Fabric oficial exige 4.5.0+ para RN 0.77 — a 3.x não tem suporte. O pin exato (e não `^4.5.0`) é deliberado: um range caret resolveria para 4.26+, que exige RN 0.84+. **Corrigido durante a execução:** o pin inicial era 4.13.1, mas essa versão introduziu `fabric/BottomTabs*` e `fabric/gamma/*`, cujo codegen o babel-plugin-codegen do RN 0.77 não parseia — quebra só o build web (o Metro tolera). A 4.12.0 é a mais nova sem esses módulos. `safe-area-context` 4.14, `image-picker` 8.2 e `async-storage` 2.2 **mantidos** — só sobem se o build quebrar. AsyncStorage 3.x NÃO entra: o Kotlin 2 até libera, mas a API é idêntica (YAGNI) |
...
- **O2.1** `react-native@0.77.3` (exato), `@react-native/babel-preset|eslint-config|metro-config|typescript-config@^0.77.3`, `@react-native-community/cli@^16.0.0`, `react-native-screens@4.12.0` (pin exato — ver DO4).
