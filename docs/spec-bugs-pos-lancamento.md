...
| M4 | **IDOR**: `GET /challenges/:id/leaderboard` entrega ranking (nomes + desempenho) de desafio privado sem checar participação | `challenges.routes.ts:106-121` | não-membro recebe **lista vazia** (não 403): fecha o vazamento sem quebrar o fluxo de convite, que busca o ranking antes de o convidado entrar — o app publicado mostrava "Algo deu errado" com o 403 |
