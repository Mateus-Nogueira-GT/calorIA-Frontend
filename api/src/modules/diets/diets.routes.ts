...
const dietsRoutes: FastifyPluginAsyncZod = async (fastify) => {
  ...
  fastify.get('/current', {
    schema: {
      tags: ['Diets'],
      summary: 'Dieta ativa',
      description: 'Retorna o cabeçalho da dieta ativa (metas de calorias/macros). Para obter os dias completos use /diets/current/full.',
      security: [{ bearerAuth: [] }],
  ...
  fastify.get('/today', {
    schema: {
      ...
      description: 'Retorna as refeições do dia atual conforme o dia da semana. É o dado principal da tela inicial.',
      security: [{ bearerAuth: [] }],
  ...
  fastify.patch('/meals/:mealId/toggle', {
    schema: {
      tags: ['Diets'],
      summary: 'Marcar refeição como concluída',
      description: 'Alterna o estado concluído/pendente de uma refeição. Também atualiza o streak do usuário.',
      security: [{ bearerAuth: [] }],
  ...
  fastify.post('/items/:itemId/replace', {
    schema: {
      tags: ['Diets'],
      summary: 'Substituir item da dieta',
      description: 'Substitui um alimento da dieta por outro. O item original é arquivado e o novo é criado.',
      security: [{ bearerAuth: [] }],
